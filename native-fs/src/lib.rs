...
use archive::cancel::CancelManager;
use archive::traits::OverwritePolicy;
...
pub extern "system" fn Java_com_xtmanager_core_filesystem_LocalFileSystem_nativeListArchiveEntries<'local>(
    ...
    let backend = match archive::get_backend_for_path(Path::new(&path_str)) {
        Ok(b) => b,
        Err(_) => return std::ptr::null_mut(),
    };
...
pub extern "system" fn Java_com_xtmanager_core_filesystem_LocalFileSystem_nativeExtractArchive<'local>(
    ...
    let backend = match archive::get_backend_for_path(Path::new(&archive_str)) {
        Ok(b) => b,
        Err(_) => return -1,
    };
    ...
    let res = backend.extract(
        ...
        &flag,
        &|_prog| {
            // Optional progress callback invocation if listener object is provided
        },
    );

    match res {
        Ok(_) => 0,
        Err(archive::traits::ArchiveError::Cancelled) => -2,
        Err(_) => -1,
    }
}
...
pub extern "system" fn Java_com_xtmanager_core_filesystem_LocalFileSystem_nativeCompressArchive<'local>(
    ...
    let backend = match archive::get_backend_for_format(&fmt_str) {
        Ok(b) => b,
        Err(_) => match archive::get_backend_for_path(Path::new(&out_str)) {
            Ok(b) => b,
            Err(_) => return -1,
        },
    };
